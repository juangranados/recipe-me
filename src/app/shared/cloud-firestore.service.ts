...
export class CloudFirestoreService {
  ...
  private stopLoading(state: States) {
    if (state === States.ShoppingList) {
      ...
      this.recipeStore
        ...
        .subscribe(isLoading => {
          if (isLoading) {
            this.recipeStore.dispatch(new recipeActions.RecipeStopLoading());
          }
        });
    }
  }
  private startLoading(state: States) {
    if (state === States.ShoppingList) {
      ...
      this.recipeStore
        ...
        .subscribe(isLoading => {
          if (!isLoading) {
            this.recipeStore.dispatch(new recipeActions.RecipeStartLoading());
          }
        });
    }
  }
  ...
  public addElement<T>(element: T, path: string, state: States) {
    ...
    this.afs
      ...
      .add(element)
      .then(docRef => {
        this.messageService.setMessage('Elemento guardado en Cloud Firestore');
        this.stopLoading(state);
      })
      .catch(error => {
        this.messageService.setMessage(
          'Error al guardar el elemento en Cloud Firestore: ' + error.message
        );
        this.stopLoading(state);
        this.registerError(
          state,
  ...
  deleteElement(id: string, path: string, state: States) {
    ...
    this.afs
      ...
      .delete()
      .then(() => {
        this.messageService.setMessage('Elemento eliminado de Cloud Firestore');
        this.stopLoading(state);
      })
      .catch(error => {
        this.messageService.setMessage(
          'Error al borrar el elemento en Cloud Firestore: ' + error.message
        );
        this.stopLoading(state);
        this.registerError(
          state,
  ...
  editElement<T>(id: string, element: T, path: string, state: States) {
    ...
    this.afs
      ...
      .catch(error => {
        this.messageService.setMessage(
          'Error al editar el elemento en Cloud Firestore: ' + error.message
        );
        this.stopLoading(state);
        this.registerError(
          state,
      ...
      .subscribe(
        ...
        (data: T[]) => {
          ...
          if (data.length !== 0) {
            ...
            this.editElement(data[0]['id'], finalElement, path, state);
          } else {
            this.addElement(element, path, state);
          }
        }
