...
export function shoppingListReducer(state = initialState, action: actions.ShoppingListActions) {
    ...
    switch (action.type) {
        ...
        case actions.INGREDIENT_ADDED: // Se añade un ingrediente mediante la función addOne de EntityState.
            return shoppingListAdapter.addOne(
                action.payload,
        ...
        case actions.INGREDIENT_MODIFIED: // Se modifica un ingrediente mediante la función updateOne de EntityState.
            return shoppingListAdapter.updateOne({ // Se genera un objeto representando el nuevo estado para borrar el campo error (de tenerlo).
        ...
        case actions.INGREDIENT_REMOVED: // Se borra un ingrediente mediante la función removeOne de EntityState.
            return shoppingListAdapter.removeOne(action.payload.id, { // Se genera un objeto representando el nuevo estado para borrar el campo error (de tenerlo).
                ids: state.ids,
                entities: state.entities,
                isLoading: state.isLoading
            });
...
export const getError = createSelector(getShoppingListState, (state: State) => state.error); // Selector de error.
export const getIsLoading = createSelector(getShoppingListState, (state: State) => state.isLoading); // Selector de isLoading
export const getIngredientById = (id) => createSelector(selectEntities, (entities) => entities[id]);
