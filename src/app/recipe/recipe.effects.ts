  ...
  query$: Observable<Action> = this.actions$ // Sintaxis por defecto.
    ...
    .pipe(
      switchMap(() => {
        ...
        this.store.dispatch(new recipeActions.RecipeStartLoading()); // Se cambia el estado para reflejar que se inicia una operación asíncrona en Firebase.
        return this.afs
      ...
      map(action => {
        ...
        this.store
          ...
          .subscribe((isLoading: Boolean) => {
            if (isLoading) {
              this.store.dispatch(new recipeActions.RecipeStopLoading()); // Se informa al estado de que para la carga al recibirse los datos de Cloud Firestore.
            }
          });
